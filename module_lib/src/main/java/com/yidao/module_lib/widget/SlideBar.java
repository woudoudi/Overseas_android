...
public class SlideBar extends View {
    ...
    protected void onDraw(Canvas canvas) {
        ...
        int topOffset = (getHeight()-(letters.length*singleHeight))/2;//获取父控件的高度减去字母总长度/2；目的是让字母在屏幕中间
        ...
        for(int i=0;i<len;i++){
            ...
            //要画的字母的x,y坐标
            float x = width/2;
            float y = singleHeight*(i+1)- paint.measureText(letters[i])/2 + topOffset;
            //画字母
    ...
    public boolean dispatchTouchEvent(MotionEvent event) {
        ...
        int topOffset = (getHeight()-(letters.length*36))/2;

        //当前选中字母的索引
        final int index = (int)((event.getY()-topOffset)/(letters.length*36) * letters.length);
        //老的索引
